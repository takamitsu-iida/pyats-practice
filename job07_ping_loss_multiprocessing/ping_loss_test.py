...
class ping_loss_test_class(aetest.Testcase):
                ...
                for intf in interfaces:
                    ...
                    with dev_step.start(intf, continue_=False) as intf_step:
                        ...
                        with intf_step.start(f'{intf} down', continue_=False) as intf_down_step:
                            ...
                            p = Process(name='ping', target=ping, args=(child_conn, ping_dev, pinger['to']), kwargs={'repeat': 100000})
                            p.start()

                            # 同時にターゲット装置でインタフェースを閉塞
                            # shutの引数はこの通り
                            # def shut(dev, intf_name):
                            shut(dev, intf)

                            # 閉塞したインタフェース上でOSPFネイバーがダウンするまで待機
                            if verify_ospf_neighbor(dev, intf, False):
                                # ネイバーが見えなくなって5秒待機してから、
                                time.sleep(5)
                        ...
                        with intf_step.start(f'{intf} up', continue_=False) as intf_up_step:
                            ...
                            if verify_ospf_neighbor(dev, intf, True):
                                time.sleep(5)
                                parent_conn.send('CTRL-SHIFT-6')
                            else:
    ...
    def dump_result(self):
        """
        self.ping_loss_resultに保存されているping欠損値を表に変換してファイルに保存
        """
