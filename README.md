...
・試験結果報告書（納品物）をデジタルに
  設計書に書ききれないものは、事前検証環境をモックにして状態を保全、それも納品物にすればいい。
  いつでもモックからshow config、show interface、show spanning-treeなど、取り出せる。

・立会者（ダブルチェック者）のもっと機能させる
  （作業者だけでなく）関係者が等しく状況を把握できることが重要
  pyATSの実行結果はWebexに通知することもできる、ブラウザで結果を確認
...
pyATSの最大の弱点はPythonのスキルがないと手も足も出ないこと

これまで：　状況に応じて適切なコマンドを打ち込む　というお仕事から
これから：　ツールで収集したデータの中から情報を発掘する、という仕事に変わる

人材育成の DevNet に参加しましょう
１年後、２年後には違う世界がみえてるかもしれませんよ
どの誤家庭にも転がってるCatalystで試してみたらいかがでしょうか。
ニーズがあればもくもく会をやろうと思います。
